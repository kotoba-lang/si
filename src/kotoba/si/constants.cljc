(ns kotoba.si.constants
  "Physical and empirical constants for the `kotoba.si.*` signal-integrity
  formulas, extracted from kami-si's (retired Rust crate,
  kotoba-lang/kami-engine, ADR-2607010000) `const` definitions and inline
  magic numbers into pure EDN data
  (`resources/kotoba/si/constants.edn`).
  ...
  so it inlines a literal map instead of reading the resource. That mirror is
  equivalent EXCEPT for the three `:eye/lcg-*` constants, which need more than
  53 bits and are omitted rather than rounded -- see the comment at the
  omission."
  #?@(:clj [(:require [clojure.edn :as edn]
                       [clojure.java.io :as io])]))

(def constants
  "All kotoba.si constants, namespaced by domain (`:tline/*` `:crosstalk/*`
  `:eye/*`). See `resources/kotoba/si/constants.edn` for provenance
  comments tying each value back to the Rust source it was extracted from."
  #?(:clj (edn/read-string (slurp (io/resource "kotoba/si/constants.edn")))
