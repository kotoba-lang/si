(ns kotoba.si.crosstalk
  ...
  (:require [kotoba.si.constants :as constants]
            [kotoba.si.math :as math]))
...
(defn analyze-crosstalk
  ...
  `kotoba.si.transmission-line/calculate-z0`); `coupling-length-mm` and
  `spacing-mm` describe the coupled-line geometry; `rise-time-ps` is the
  aggressor signal's rise time.
